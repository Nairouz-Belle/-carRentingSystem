...
<a href="{{route('manager.company.index')}} "  style="color: black;"><i class="ri-image-edit-line align-bottom me-1"></i> @lang('messages.Go Back')</a>
</label>
</div>
</div>
</div>
...
<form action="{{ route('manager.company.update',$company->id) }}" method="POST" enctype="multipart/form-data">
@csrf
@method('PUT')
...
<input type="text" name="twitter" value="{{$company->twitter}}" class="form-control" required />
</div>
</div>
...
<input type="text" name="linkedin" value="{{$company->linkedin}}" class="form-control"/>
</div>
</div>
