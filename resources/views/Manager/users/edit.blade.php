...
<a href="{{route('manager.users.index')}} "  style="color: black;"><i class="ri-image-edit-line align-bottom me-1"></i> @lang('messages.Go Back')</a>
</label>
</div>
</div>
</div>
...
<form form action="{{ route('manager.users.update',$user->id) }}" method="POST" enctype="multipart/form-data" style="display: flex; gap:20px;">
@csrf
@method('PUT')
<div class="col-xxl-3">
...
<h5 class="fs-17 mb-1 text-primary" style="font-weight:bold;">{{$user->name}}</h5>
<p class="text-warning mb-0 text-uppercase"style="font-weight:bold;">{{$user->type}}</p>
</div>
</div>
...
<div class="col-lg-6">
    <div class="mb-3">
        ...
        <select class="form-control text-success "style="font-weight: bold;" data-trigger name="type" required id="roleSelect">
           
            <option value="customer" class="text-muted" {{($user->type == 'customer') ? 'selected' : ''}}> @lang('messages.Customer') </option>
            <option value="manager" class="text-muted" {{($user->type == 'manager') ? 'selected' : ''}}> @lang('messages.Manager') </option>
...
@if($user->gender ==='Male')
<select class="form-control text-primary" style="font-weight:bold;" data-trigger name="gender" name="gender" required id="">
<option value="Male" class="text-primary" {{($user->gender ==='Male') ? 'selected' : ''}}> @lang('messages.Male') </option>
<option value="Female" class="text-danger"{{($user->gender ==='Female') ? 'selected' : ''}}> @lang('messages.Female') </option>
</select>

@else
<select class="form-control text-danger" style="font-weight:bold;" data-trigger name="gender" name="gender" required id="">
<option value="Male" class="text-primary" {{($user->gender ==='Male') ? 'selected' : ''}}> @lang('messages.Male') </option>
<option value="Female" class="text-danger"{{($user->gender ==='Female') ? 'selected' : ''}}> @lang('messages.Female') </option>
</select>
@endif
...
<select class="form-control text-warning" style="font-weight: bold;" name="status" id="" >

<option  class="text-muted" style="font-weight: bold;" value="Pending" {{($user->status ==='Pending') ? 'selected' : ''}}>@lang('messages.Pending')</option>
<option  class="text-muted" style="font-weight: bold;" value="Confirmed" {{($user->status ==='Confirmed') ? 'selected' : ''}}>@lang('messages.Confirmed')</option>
<option  class="text-muted" style="font-weight: bold;" value="Blocked" {{($user->status ==='Blocked') ? 'selected' : ''}}>@lang('messages.Blocked')</option>

</select>
@elseif($user->status=='Confirmed')
<select class="form-control text-success" style="font-weight: bold;" name="status" id="" >
...
@else
<select class="form-control text-danger" style="font-weight: bold;" name="status" id="" >
